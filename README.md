...
<div id="header">
  <div class="header-top">
    ...
    <div class="header-btns">
      <span id="adminBadge" class="admin-badge" onclick="handleAdminBadge()" style="display:none">🔓 ADMIN</span>
      <button class="btn-ghost" id="adminBtn" onclick="openPinModal()">🔐 ADMIN</button>
      <button class="btn-purple" id="manageCatsBtn" onclick="openCatsModal()" style="display:none">🏷 CATS</button>
      <button class="btn-ghost" id="peopleBtnHeader" onclick="openPeopleModal()" style="display:none">👥 PEOPLE</button>
      <button class="btn-orange" id="addItemBtn" onclick="openAddItemModal()" style="display:none">+ ITEM</button>
    </div>
  </div>
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
  <div class="subtitle" id="subtitle" style="margin-bottom:0">—</div>
  ...
  <div class="top-controls">
    <input id="search" type="text" placeholder="🔍  Search gear..." oninput="render()">
...
<div id="footer">
  <button class="reset-btn" onclick="resetAll()">↺ Return all gear — end of day</button>
</div>

<!-- PIN MODAL -->
<div class="overlay" id="overlayPin" onclick="closePinModal()">
  <div class="modal" onclick="event.stopPropagation()">
    <div class="modal-title" id="pinTitle">🔐 Admin Login</div>
...
DB_REF.on('value', snapshot => {
  ...
  if(state.appName){ const t=document.getElementById("appLogoText"); if(t) t.textContent=state.appName.replace(/⚡\s*/g, "").trim(); }
  render();
});
...
function submitPin(){
  ...
  if(entered===state.pin){
    ...
    document.getElementById("pinError").textContent="❌ Wrong PIN"; clearPinInputs(); document.getElementById("p0").focus();
  }
}
function changePin(){
  const np=getPin("np");
  ...
  addLog("Admin","—","—","pin_changed",0);
  alert("✓ PIN changed!"); for(let i=0;i<4;i++) document.getElementById("np"+i).value="";
}
...
function renderGear(search,filterCat){
  ...
  if(!list.length){ listEl.innerHTML='<div class="empty">No gear found. 🎬</div>'; return; }
  ...
  listEl.innerHTML=list.map(item=>{
    ...
    const handleHTML=isAdmin?`
      ...
      <div class="move-btns mobile-only">
        <button class="move-btn" onclick="moveItem(${item.id},-1);event.stopPropagation()">▲</button>
        <button class="move-btn" onclick="moveItem(${item.id},1);event.stopPropagation()">▼</button>
      </div>`:"";
    ...
    const catEl=isAdmin
      ?`<span class="cat-tag editable" style="background:${color}22;color:${color};border:1px solid ${color}44" title="Click to change category" onclick="openCatDropdown(event,${item.id})">${item.category} ▾</span>`
      :`<span class="cat-tag" style="background:${color}22;color:${color};border:1px solid ${color}44">${item.category}</span>`;
    const delBtn=isAdmin?`<button class="del-btn" onclick="deleteItem(${item.id})">✕</button>`:"";
...
function startCatRename(oldName, el){
  ...
  function commit(){
    ...
    addLog("Admin","—",oldName,"cat_renamed",0);
...
function deleteCategory(name){
  ...
  if(count>0){ alert(`Cannot remove "${name}" — ${count} item(s) still using it. Reassign them first.`); return; }
  ...
  listEl.innerHTML=state.people.map(person=>{
    ...
    if(!theirItems.length) return `
      <div class="person-card"><div class="person-header">
        <span class="person-name-big">👤 ${person}</span>
      ...
      <div class="person-card">
        <div class="person-header">
          ...
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            ...
            <button class="return-btn" onclick="returnAllFromPerson('${person}')" style="border-color:#00C9A7;color:#00C9A7;font-size:11px;padding:3px 10px">↺ Return all</button>
          </div>
        </div>
          ...
          ${theirItems.map(i=>`
            <div class="person-item-row">
              ...
              <div style="display:flex;align-items:center;gap:8px">
                <span style="font-size:13px;font-weight:700;color:#FF6B35">× ${i.checkouts[person]}</span>
...
function renderLog(){
  const listEl=document.getElementById("list");
  listEl.classList.remove("admin-mode");
  if(!state.log.length){ listEl.innerHTML='<div class="empty">No activity yet. 📋</div>'; return; }
  const byDay={};
  state.log.forEach(e=>{ const k=fmtDate(e.ts); if(!byDay[k]) byDay[k]=[]; byDay[k].push(e); });
  const clearBtn=isAdmin?`<button onclick="clearLog()" style="background:transparent;border:1px solid #2A2A30;color:#555;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:11px;font-family:inherit">🗑 Clear</button>`:"";
  listEl.innerHTML=`<div style="padding:10px 14px 4px;display:flex;justify-content:space-between;align-items:center">
    ...
    Object.entries(byDay).map(([day,entries])=>`
      <div class="log-day"><div class="log-day-title">📅 ${day}</div>
        ${entries.map(e=>{
          ...
          let txt;
          if(e.action==="out") txt=`<span class="log-qty-out">took ${e.qty}×</span> <span style="color:#F0EDE8">${e.itemName}</span>`;
          else if(e.action==="in") txt=`<span class="log-qty-in">returned ${e.qty}×</span> <span style="color:#F0EDE8">${e.itemName}</span>${e.project?` <span style='color:#555'>· ${e.project}</span>`:""}` ;
          ...
          else if(e.action==="stock_removed") txt=`<span class="log-qty-admin">−${e.qty} stock</span> <span style="color:#F0EDE8">${e.itemName}</span>`;
          ...
          return `<div class="log-entry">
            ...
            <div class="log-body">
              <div class="log-person">${isAdm?"🔐 Admin":"👤 "+e.person}</div>
              <div class="log-action">${txt}</div>
            </div>
            ${isAdmin?`<button onclick="deleteLogEntry(${e.id})" style="background:transparent;border:none;color:#333;font-size:14px;cursor:pointer;padding:2px 6px;flex-shrink:0;line-height:1" onmouseover="this.style.color='#ff4444'" onmouseout="this.style.color='#333'">✕</button>`:""}
            </div>`; }).join("")}
      </div>`).join("");
}
...
function saveReturnDate(person, val){
  ...
  if(val){
    ...
    addLog(person,"—",person,"return_date_set",0,val);
  } else {
...
function saveProject(person, val){
  ...
  if(trimmed){
    ...
    addLog(person,"—",trimmed,"project_set",0,"");
  } else {
...
function renderPeopleList(){
  document.getElementById("peopleList").innerHTML=state.people.map(p=>`
    <div class="person-row" id="prow-${p.replace(/[^a-z0-9]/gi,'_')}">
      <span style="font-size:14px;cursor:pointer;border-bottom:1px dashed #444" onclick="startPersonRename('${p}',this)" title="Click to rename">👤 ${p}</span>
...
function startPersonRename(oldName, el){
  ...
  function commit(){
    ...
    addLog("Admin","—",oldName,"person_renamed",0);
